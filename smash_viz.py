...
_p_chr = (_float("START") + _float + pp.Literal("m") + pp.Literal("glx") + pp.Literal("sp") +
          _float + _float + pp.Literal("m") +
          pp.Literal("(") + (pp.Word(pp.nums) | pp.Literal("X") | pp.Literal("Y"))("CHR") + pp.Literal(")") +
          pp.Literal("jc") + pp.Literal("s"))


def main():
    parser = argparse.ArgumentParser(description='Change the visual style of SMASH PostScript plots.')
    parser.add_argument('--ps', '--input', type=argparse.FileType('r'),
                        required=True,
